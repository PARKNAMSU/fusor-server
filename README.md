...
```bash
fuser-server$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
fuser-server$ sam local start-api
fuser-server$ curl http://localhost:3000/
```
...
```bash
fuser-server$ sam logs -n HelloWorldFunction --stack-name fuser-server --tail
```
...
```bash
sam delete --stack-name fuser-server
```
